...
function Menu({startingItems}) {
    // Setting the value of the "startingItems" props (AKA The "menuItems" array) as the starting value for the "itmes" state
    ...
    function updateQty(itemId, newQty){
        // A new update copy of the itmes (AKA menuItems) array is created and set to the "newItems" variable by mapping through the current value of the array and updating the qty (Quantity) value for the object (AKA Type of pizza) that was selected.
        const newItems = items.map(i => {
            ...
            if(i.id === itemId){
                // Creating a new object and spreading the properties in the array/object that is currently being passed into this function (AKA the function for the map method). Also, updating the value of the qty property by overriding the value for that property. Finally, returnning the new object so it will be added as an item in the new array that is returned from using the map method.
                return {...i, qty: newQty}
            }
            // Returning the item/object without making any change to it.
            return i;
        })
        // Setting the new array value (AKA "newItems" variable) that was returned from the map method as the new state value for "itmes". Also, because the "setItems" state function is used to updated the value for "items" the Menu component re-renders which causes the three MenuItem components to being created (AKA Rendered) again as well.
        setItems(newItems);
    }
    ...
    return(
        <section className="Menu">
            <h1 className="Menu-title">Lee's Pizzeria</h1>
            <div>
                {/* <MenuItem 
                ...
                {items.map(i => (
                    <MenuItem 
                    // Keys help React identify which items have been changed, are added, or are removed. Keys should be given to the elements inside the array to give the elements a stabled identity.
                    key={i.id}
                    id={i.id}
                    type={i.type}
                    price={i.price}
